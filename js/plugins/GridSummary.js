...
Ext.extend(Ext.ux.grid.GridSummary, Ext.util.Observable, {
    init : function(grid) {
        ...
        if (!this.rowTpl) {
            this.rowTpl = new Ext.Template(
                '<div class="x-grid3-summary-row2 x-grid3-gridsummary-row-offset2">',
        ...
        if (!this.cellTpl) {
            this.cellTpl = new Ext.Template(
                '<td class="x-grid3-col x-grid3-cell x-grid3-td-{id} {css}" style="{style}">',
                    '<div class="x-grid3-cell-inner2 x-grid3-col-{id}" unselectable="on" {attr}>{value}</div>',
                "</td>"
            );
    ...
    onLayout : function(vw, vh) {
        ...
        if (!this.grid.getGridEl().hasClass('x-grid-hide-gridsummary2')) {
    ...
    renderSummary : function(o, cs, cm) {
        ...
        for (var i = 0, len = cs.length; i < len; i++) {
            ...
            p.id = c.id;
            p.style = c.style;
            p.css = i === 0 ? 'x-grid3-cell-first2 ' : (i == last ? 'x-grid3-cell-last ' : '');
    ...
    refreshSummary : function() {
        ...
        if (!this.view.summaryWrap) {
            this.view.summaryWrap = Ext.DomHelper.insertAfter(this.view.scroller, {
                tag: 'div',
                cls: 'x-grid3-gridsummary-row-inner2'
            }, true);
        }
    ...
    toggleSummary : function(visible) { // true to display summary row
        ...
        if (el) {
            if (visible === undefined) {
                visible = el.hasClass('x-grid-hide-gridsummary2');
            }
            el[visible ? 'removeClass' : 'addClass']('x-grid-hide-gridsummary2');
